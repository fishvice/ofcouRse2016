...
And attempt to mimic a report now generated via java script provided by Fiskistofa. The basis of the mimicry is [Elliðiaárskýrsla 2015](http://www.hafro.is/~einarhj/education/ofcouRse2016/ellidaar_veidiskyrsla2015.pdf). The source set we use is the [Excel-sheet](http://www.hafro.is/~einarhj/education/ofcouRse2016/data/ellidaar.xlsx) we used on Day 1, as well one additional [data set](http://www.hafro.is/~einarhj/education/ofcouRse2016/data/lw.csv) that is used to convert length to weight, if weight is not given.
...
x <-
  d2 %>% 
  mutate(Þyngdarbil = cut(thyngd_est, c(seq(1, 5.5, 0.5), 8))) %>% 
  group_by(Þyngdarbil) %>% 
  summarise(Fjöldi = n()) %>% 
  ungroup() %>% 
